...
class VivenuHttpxAPI:
    ...
    async def _ensure_client(self):
        ...
        if self._client is None:
            try:
                ...
                logger.warning(f"Failed to create advanced httpx client: {e}")
                logger.info("Falling back to simple httpx client configuration")
    ...
    async def _get_events_async(self):
        """Async implementation of get_events using httpx with retry logic"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(f"Httpx request failed (attempt {attempt + 1}/{max_retries}): {str(e)}")
                    logger.info(f"Retrying in {delay} seconds...")
                    await asyncio.sleep(delay)
                else:
                    logger.error(f"Httpx request failed after {max_retries} attempts: {str(e)}")
                    raise
