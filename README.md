...
## Gender Analysis - Complete Workflow

Collection Stage

1. **Analysis Complete**: Must have run `ingest_gender_fix.py` first to generate payloads
2. **Environment Variables**: Same setup as the analysis script
3. **Database Access**: PostgreSQL connection configured
4. **API Access**: Valid Vivenu API token with update permissions

Updating Stage

The `update_gender_fields.py` script:
1. **Reads** gender update payloads from JSON files generated by `ingest_gender_fix.py`
2. **Sends** PUT requests to the Vivenu API to update gender fields
3. **Tracks** update progress and results in the database
4. **Provides** detailed logging and error handling
5. **Supports** dry-run mode for testing without making actual changes


```bash
# 1. Run analysis (generates payloads)
docker exec -it vivenu-app python ingest_gender_fix.py
# OR
docker exec -it vivenu-app python ingest_gender_fix.py --debug
# OR
# Force fresh start (explicitly drop and recreate table)
docker exec -it vivenu-app python ingest_gender_fix.py --force-fresh

# 2. Review generated payloads
ls -la data/gender_update_payloads_*.json

# 3. Test with dry-run
docker exec -it vivenu-app python  update_gender_fields.py --payload-file data/gender_update_payloads_perth_20241201_143022.json --dry-run

# 4. Run actual updates (when ready)
docker exec -it vivenu-app python  update_gender_fields.py --payload-file data/gender_update_payloads_perth_20241201_143022.json
